...
Handles object detection using YOLOv8 and triggers ESP32 alerts
"""
    ...
    def detect(self, frame):
        ...
        if results and len(results) > 0:
            ...
            for i in range(len(boxes)):
                ...
                # Send alert to ESP32 if abnormal event detected
                if label.lower() in ['person', 'dog', 'cat', 'horse', 'pothole']:  # add class names of interest here
                    self.send_alert_to_esp32("ALERT")
                    print(f"Alert triggered for: {label}")

        return detections

    def send_alert_to_esp32(self, message):
        """Send alert message to ESP32 over Wi-Fi TCP"""
        esp_ip = '192.168.150.87'   # replace with actual ESP32 IP
        esp_port = 3333
        try:
